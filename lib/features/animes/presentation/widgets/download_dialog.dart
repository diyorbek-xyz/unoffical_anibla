...
class _DownloadMenuState extends State<DownloadMenu> {
  ...
  AnimeEntity get anime => _animeController.mediaState.value.value!;
  SeasonEntity get season => _animeController.seasonsState.value.value!.first;
  ...
  void downloadEpisode(Variant variant) async {
    ...
    final dwTask = await downloader.download(
      DownloadInfos(
        animeModel: AnimeMapper.entityToModel(anime),
        episodeModel: EpisodeMapper.entityToModel(widget.episode),
        seasonModel: SeasonMapper.entityToModel(season),
        variant: variant,
        masterPlaylist: masterPlaylist,
        downloadUrl: masterPlaylist!.downloadUrl,
        size: 0,
      ),
    );
    setState(() => [task = dwTask, isLoading = false]);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      content: Column(crossAxisAlignment: .stretch, children: [mainBuilder(), actions()]),
    );
  }

  Widget mainBuilder() => Expanded(
    child: Builder(
      builder: (context) {
        final isMobile = MediaQuery.of(context).size.width < MOBILE_WIDTH;
        ...
        if (!isNotReady) return variantsBuilder();
  ...
  Widget variantsBuilder() => Column(
    ...
    children: [
      ...
      Expanded(
        child: ListView(
          children: (masterPlaylist?.variants ?? [])
              .map(
                (e) => ListTile(
