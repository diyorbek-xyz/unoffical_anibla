  ...
  Widget build(BuildContext context) {
    return SignalBuilder(
      builder: (context) {
        ...
        if (state.isLoading) return SliverToBoxAdapter(child: CircularProgressIndicator());
        if (state.hasError) return SliverToBoxAdapter(child: Text((state.error as Failure).message));
        final data = state.value ?? profileController.fakeNotification;
        return ListsWidget.builder(
